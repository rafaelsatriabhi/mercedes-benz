...
import introductionStyles from './introduction-section-styles.module.css';
...
const IntroductionSection = ({ setShowModal }) => {
  ...
  return (
    <>
      <section className={`${introductionStyles.animation} w-full 96`} id="carousel-section">
        <div className="flex flex-col laptop:flex-row justify-center laptop:justify-between w-full h-full bg-gradient-to-t laptop:bg-gradient-to-r from-black">
          ...
          <div className="w-full laptop:w-4/12 pl-3.5 laptop:pr-16 flex flex-col items-center justify-center bottom-0 laptop:bottom-28">
            <div aria-hidden role="button" onClick={() => setShowModal(true)} className="text-center bg-red-800 w-full py-4 mb-5 transition duration-500 ease-in-out bg-red-600 hover:bg-blue-600 transform hover:-translate-y-1 hover:scale-110 rounded-md">Berikan Saya Penawaran Terbaik</div>
            <div onClick={() => router.push('https://api.whatsapp.com/send/?phone=6281285655853&text&app_absent=0?lang=id')} aria-hidden role="button" className="border-4 border-green-600 hover:bg-green-200 hover:text-black flex items-center justify-center w-full py-4 rounded-md text-center">
